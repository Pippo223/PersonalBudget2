 ...
 */
 transactionRouter.post('/', async (req, res) => {
    try {
        const { description, payment_amount, envelope_id } = req.body;
        const date = new Date();
        await pool.query('BEGIN');
        const envelope = await pool.query('SELECT * FROM enveloppes WHERE id = $1', [envelope_id]);
        if (envelope.rowCount < 1) {
            return res.status(404).send({
                message: "There is no enveloppe with this id."
            });
        };
        const newTransaction = await pool.query('INSERT INTO transactions (date, description, payment_amount, envelope_id) VALUES ($1, $2, $3, $4) RETURNING *', 
        [date, description, payment_amount, envelope_id]);
        await pool.query('UPDATE envelopes SET budget = budget - $1 where id = $2 RETURNING *',
